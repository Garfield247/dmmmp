...
@usergroup.route("/info/", methods=["GET"], defaults={"desc": {"interface_name": "获取用户组信息",
                                                               "is_permission": True,
                                                               "permission_belong": 2}})
def info(desc):
    '''
...
@usergroup.route("/post/", methods=["POST", "PUT"], defaults={"desc": {"interface_name": "添加编辑用户组信息",
                                                                       "is_permission": True,
                                                                       "permission_belong": 2}})
def post_group(desc):
    '''
    ...
    if request.method == 'POST' and dmp_group_id == None:
        try:
            ...
            db.session.commit()
            ret_data = EnvelopedData.post_edit(group_obj, dmp_permission_list, dmp_group_name)
    ...
    # 编辑用户组信息
        ...
        try:
            ...
            ret_data = EnvelopedData.post_edit(edit_group_obj, dmp_permission_list, dmp_group_name)
...
@usergroup.route("/del/", methods=["DELETE"], defaults={"desc": {"interface_name": "删除用户组",
                                                                 "is_permission": True,
                                                                 "permission_belong": 2}})
def ugdel(desc):
    '''
