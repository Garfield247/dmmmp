...
def info(desc):
    if request.method == "GET":
        ...
        try:
            committed, pending, complete = {"form_status_code": 1}, {
                "form_status_code": 2}, {"form_status_code": 3}
            if form_permission(user_id) == 1:
                ...
                if form_status == 2 or form_status == 0:
                    pending_query = Forms.query.filter(and_(tuple_(Forms.submit_dmp_user_id,).in_(Users.query.with_entities(
                        Users.id).filter_by(leader_dmp_user_id=user_id).all()), Forms.approve_result == 0))
                    pending["form_count"] = pending_query.count()
                    pending["page_num"] = page_num
                    pending["forms"] = [f.__json__() for f in pending_query.limit(
                        page_limit).offset((page_num-1)*page_limit)]
                if form_status == 3 or form_status == 0:
                    complete_query = Forms.query.filter(and_(or_(
                        Forms.submit_dmp_user_id == user_id, Forms.approve_dmp_user_id == user_id), Forms.approve_result != 0))
                    complete["form_count"] = complete_query.count()
                    complete["page_num"] = page_num
                    complete["forms"] = [f.__json__() for f in complete_query.limit(
                        page_limit).offset((page_num-1)*page_limit)]
...
def dlfunc(meta):
    ...
    files = [os.path.join(download_path, f) for f in os.listdir(
        download_path) if f.startswith(file_name)]
    if len(files) == 1:
        ...
        try:
            ...
            if form_type == 1:
                ...
                file_path = os.path.join(
                    upload_path, approve_form.info_form.filepath)
                ...
                try:
                    ...
                    if destination_database_type == 1:
                        # hive_writer
                        hive_columns = [{"name": col, "type": "string"}
                        ...
                        current_app.logger.error(create_table_sql)
                        if method == 1:
                            hive_conn.execsql(create_table_sql)
                        elif method == 3:
                            del_table_sql = "drop table {table_name}"
                            hive_conn.execsql(del_table_sql.format(
                        ...
                        if method == 1:
                            ...
                            mysql_conn.execsql(del_table_sql.format(
                                table_name=destination_db_table_name))
                    ...
                    job_hanlder.delay(reader=reader, writer=writer,
                                      channel=3, func=postfunc, meta=meta)
                    approve_form.result = "SUCCEED!"
                except Exception as err:
                ...
                if approve_result == 1:
                    ...
                    try:
                        base_column = auto_connect(db_id=origin_data_table.dmp_database_id).columns(
                        ...
                        if origin_database_type == 1:
                            ...
                            # mysql_reader
                            column = [col.get("dmp_data_table_column_name")
                        ...
                        if destination_database_type == 1:
                            # hive_writer
                            hive_col = [col.get("dmp_data_table_column_name")
                                        for col in base_column]
                            hive_columns = [
                                {"name": col, "type": "string"} for col in hive_col]
                            hive_path = "/user/hive/warehouse/%s.db/%s" % (
                                destination_db_name, destination_db_table_name)
                            hive_conn = auto_connect(
                ...
                if approve_result == 1:
                    ...
                    if origin_data_table:
                        ...
                        try:
                            ...
                            headers = [col.get("dmp_data_table_column_name")
                                       for col in base_column]
                            writer = textfile_writer(
                                filepath=download_path, filename=file_name, header=headers)
                            ...
                            job_hanlder.delay(
                                reader=reader, writer=writer, channel=1, func=dlfunc, meta=meta)
                        except Exception as err:
