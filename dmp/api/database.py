...
@database.route("/info/", methods=["GET"], defaults={"desc": {"interface_name": "数据库信息",
                                                              "is_permission": True,
                                                              "permission_belong": 1}})
def info(desc):
...
@database.route("/del/", methods=["DELETE"], defaults={"desc": {"interface_name": "删除数据库连接信息",
                                                                "is_permission": True,
                                                                "permission_belong": 2}})
def dbdel(desc):
...
@database.route("/connect/", methods=["POST"], defaults={"desc": {"interface_name": "测试数据库连接",
                                                                  "is_permission": True,
                                                                  "permission_belong": 1}})
def connect(desc):
...
@database.route("/post/", methods=["POST", "PUT"], defaults={"desc": {"interface_name": "添加/修改数据库信息",
                                                                      "is_permission": True,
                                                                      "permission_belong": 1}})
def post(desc):
    db_info = request.json
